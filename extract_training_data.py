...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        # convert input/output pairs into repr. for neural network
        # takes in list of words, list of POS tags, state
        
        li = [] # contains indices of words
        
        # stackloop
        for i in range(1,len(state.stack)):
            # if stack has <=2 words, need to pad using NULL
            if len(state.stack)<i: # <NULL> 4
                li.append(self.word_vocab["<NULL>"])
            # add ROOT if top elem in stack is None
            elif words[state.stack[-i]] is None: # <ROOT> 3
                li.append(self.word_vocab["<ROOT>"])
            else:
                if pos[state.stack[-i]] == "CD": # <CD> 0
                    li.append(self.word_vocab["<CD>"])
                elif pos[state.stack[-i]] == "NNP": # <NNP> 1
                    li.append(self.word_vocab["<NNP>"])
                elif words[state.stack[-i]].lower() in self.word_vocab: # edge cases w diff capitalization
                    li.append(self.word_vocab[words[state.stack[-i]].lower()]) 
                else:
                    li.append(self.word_vocab["<UNK>"])
                    
        # buffer loop, similar to the stack loop
        for i in range(1,len(state.buffer)):
            if len(state.buffer)<i: # <NULL> 4
                li.append(self.word_vocab["<NULL>"])
            elif words[state.buffer[-i]] is None:
                li.append(self.word_vocab["<ROOT>"])
            else:
                if pos[state.buffer[-i]] == "CD": # <CD> 0
                    li.append(self.word_vocab["<CD>"])
                elif pos[state.buffer[-i]] == "NNP": # <NNP> 1
                    li.append(self.word_vocab["<NNP>"])
                elif words[state.buffer[-i]].lower() in self.word_vocab: # edge cases w diff capitalization
                    li.append(self.word_vocab[words[state.buffer[-i]].lower()]) 
                else:
                    li.append(self.word_vocab["<UNK>"])        
        
        # return an encoding of the input to the neural network as a single vector
        if len(li)==6:
            return np.array(li)
        else:
            return np.zeros(6)
    ...
    def get_output_representation(self, output_pair):  
        ...
        # there are 45*2+1=91 possible outputs
        output = np.zeros(91)
        
        trans = self.output_labels[output_pair]
        output[trans]=1
        
        # return a one-hot vector of length 91
        return output
